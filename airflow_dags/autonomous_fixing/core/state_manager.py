...
class ProjectStateManager:
    ...
    def save_state(self, phase: str, data: dict) -> None:
        ...
        # Prepare validation markers
        if phase == "hooks":
            precommit_config = self.project_path / ".pre-commit-config.yaml"
            git_hook = self.project_path / ".git" / "hooks" / "pre-commit"
            validation_markers = f"""- Hook file exists: {git_hook.exists()}
- Config file exists: {precommit_config.exists()}
- Last verified: {timestamp}"""
            invalidation_triggers = """- .pre-commit-config.yaml modified
- .git/hooks/pre-commit deleted
- State age > 30 days"""
        else:  # tests
            package_json = self.project_path / "package.json"
            validation_markers = f"""- Config file exists: {package_json.exists()}
- Last verified: {timestamp}"""
            invalidation_triggers = """- package.json modified
- State age > 30 days"""

        # Write state file in Markdown format with YAML frontmatter
        content = f"""---
generated: {timestamp}
status: {status}
...
"""
    ...
    def should_reconfigure(self, phase: str) -> Tuple[bool, str]:
        ...
        result = self._validate_state_file(state_file)
        if result is None:
            return (True, "state file corrupted (deleted)")
        stored_hash, generated = result
        ...
        # Extract generated_time for final message
    ...
    def _check_config_changes(self, phase, state_file, stored_hash):
        ...
        # Mtime check
        if config_file and config_file.exists() and config_file.stat().st_mtime > state_mtime:
    ...
    def _check_external_cache(self, phase: str) -> Tuple[bool, str]:
        ...
        if phase == "hooks":
            external_cache = self.EXTERNAL_HOOK_CACHE_DIR / f"{project_name}-hooks.yaml"
        else:  # tests
            external_cache = self.EXTERNAL_TEST_CACHE_DIR / f"{project_name}-tests.yaml"
