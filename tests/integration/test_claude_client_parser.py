...
class TestClaudeClientParserIntegration:
    ...
    def test_success_with_deprecation_warnings_in_stderr(self, mock_client):
        ...
        # Events showing successful completion
        events = [
            {'event': 'run_completed', 'outcome': 'Hook configured', 'timestamp': '2025-10-04T12:00:00'}
        ]
...
"""
        ...
        event_lines = [f'{{"event": "run_completed", "outcome": "Hook configured"}}\n', '']
    ...
    def test_failure_with_real_error_in_stderr(self, mock_client):
        ...
        # No explicit error event - should analyze stderr
        events = []
...
"""
    ...
    def test_mixed_noise_and_errors(self, mock_client):
        ...
        events = [
            {'event': 'run_completed', 'outcome': 'Test run completed', 'timestamp': '2025-10-04T12:00:00'}
        ]
...
"""
        ...
        event_lines = [f'{{"event": "run_completed", "outcome": "Test run completed"}}\n', '']
        mock_process.stdout.readline = Mock(side_effect=event_lines)
        mock_process.stderr.read = Mock(return_value=stderr)
        mock_process.stdin = Mock()
        mock_process.wait = Mock()
    ...
    def test_backward_compatibility_with_old_format(self, mock_client):
        ...
        events = [
            {'event': 'run_completed', 'outcome': 'Success', 'timestamp': '2025-10-04T12:00:00'}
        ]

        event_lines = [f'{{"event": "run_completed", "outcome": "Success"}}\n', '']
        mock_process.stdout.readline = Mock(side_effect=event_lines)
    ...
    def test_operation_type_included_in_errors(self, mock_client):
        ...
        with patch('subprocess.Popen', return_value=mock_process):
            result = mock_client.query(
                prompt="Discover tests",
                project_path="/test/project",
                prompt_type="discover_tests"
            )
    ...
    def test_events_preserved_in_response(self, mock_client):
        ...
        events_data = [
            {'event': 'stream', 'payload': {}, 'timestamp': '2025-10-04T12:00:00'},
            {'event': 'run_completed', 'outcome': 'Done', 'timestamp': '2025-10-04T12:00:05'}
        ]
